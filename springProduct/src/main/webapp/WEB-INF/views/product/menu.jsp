...
<!-- 스프링 시큐리티 표현식 
	...
	- hasAnyRole([role,role2]) : 여러 roles 중에서 하나라도 해당하는 role이 있으면 true (or 처럼)
	- principal : 인증된 사용자의 사용자 정보 (UserDetails 인터페이스를 구현한 클래스의 객체)
									customUser의 memberVO 멤버변수
	- authentication : 인증된 사용자의 인정 정보
...
<nav class="navbar navbar-expand navbar-dark bg-dark">
	...
	<div class="container">
		...
		<div style="float:right;">
			<!-- 인증된 사용자인 경우 true. 인증 되었을 때만(즉 로그인 했을 때만!) 이 버튼이 보여랏 !! -->
			<sec:authorize access="isAuthenticated()">
				<form action="/logout" method="post">
					<button type="submit" class="btn btn-outline-light">logout</button>
					<sec:csrfInput/>
				</form>
				<button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#exampleModal" data-bs-whatever="@mdo">나의 정보 보기</button>
			</sec:authorize>
