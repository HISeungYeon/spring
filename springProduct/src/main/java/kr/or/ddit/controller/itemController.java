...
public class itemController {
	...
	public String register(ItemVO itemVO, Model model) {
		...
		//itemVO는?? ItemVO [itemId=0, itemName=오잉, price=3000, description=뇸뇸, 
		// pictureUrl=null, pictureUrl2=null, 
		// picture=org.springframework.web.multipart.support.StandardMultipartHttpServletRequest$StandardMultipartFile@2d9bd182, itemAttachVOList=null]
		log.info("itemVO는?? " + itemVO.toString());
		...
		model.addAttribute("msg", "등록이 완료되었숩니다");
	...
	// <input type="file" name="uploadFile" multiple /> 다중파일올리기!
	// 중요!!  name="uploadFile"과 MultipartFile[] uploadFile 이름이 똑같아야 함!!!!
	public String uploadFormAction(MultipartFile[] uploadFile, Model model) {
		//MultipartFile : 스프링에서 제공해주는 타입 (스프링이 설치 되어있어야 함!!)
		/* 잘 쓰는 방법
		 * String 변수명 = String getOriginalFileName() : 업로드 되는 파일 이름(실제 파일명)
		 * boolean 변수명 = file.isEmpty() : 파일이 없다면 true
		 * long 변수명 = file.getSize() : 업로드 되는 파일의 크기
		 * file.transferTo(File file): 파일을 저장할 때
		 * 
		 * 잘 안쓰는 방법
		 * String 변수명 = file.getName()
		 * byte[] 변수명 = file.getBytes() : byte[]로 파일 데이터 변환
		 * InputStream 변수명 = getInputStream() : 파일 데이터와 연결된 InputStream을 반환
		 */
		
		//make folder 시작  연/월/일--------------------------
		File uploadFolder = new File(uploadPath, getFolder());
		log.info("uploadFoledr : " + uploadFolder);
		
		//만약 upload > 연>월>일 폴더가 없다면 생성
		if(uploadFolder.exists()==false) {
			uploadFolder.mkdirs();
		}
		//make folder 끝 --------------------------
		
		//파일객체 배열로부터 파일객체를 하나씩 꺼내오쟈
		for(MultipartFile multipartFile : uploadFile) {
			...
			//C:\\eclipse-jee-2020-06-R-win32-x86_64\\workspace\\springProduct\\src\\main\\webapp\\resources\\upload
			// 2022\\10\\28
	...
	public String getFolder() {
		//간단한 날짜 형식 2022-10-28
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		
		//날짜 객체 생성(javs.util패키지)
		Date date = new Date();
